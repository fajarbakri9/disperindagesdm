...
def create_footer_html(prefix=""):
    return f"""  <!-- FOOTER RESMI KEDINASAN KABUPATEN PINRANG -->
...
def update_file_footer(filepath, prefix=""):
    if not os.path.exists(filepath):
        return
    with open(filepath, "r", encoding="utf-8") as f:
        content = f.read()

    footer_str = create_footer_html(prefix)

    # 1. Hapus semua tag footer eksisting
    while "<footer" in content and "</footer>" in content:
        start = content.find("<footer")
        end = content.find("</footer>", start) + len("</footer>")
        content = content[:start] + content[end:]

    # 2. Sisipkan footer sebelum </body>
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print(f"Successfully injected exact master footer into: {filepath}")
