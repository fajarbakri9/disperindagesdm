...
def get_exact_footer(prefix=""):
    return f'''  <!-- 13. FOOTER RESMI KEDINASAN (100% IDENTIK & KONSISTEN) -->
    ...
    <div class="container">
      <div class="footer-grid">
        ...
        <div class="footer-col">
          ...
          <div style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(250, 204, 21, 0.35); border-left: 4px solid var(--accent-gold); padding: 12px 14px; border-radius: 10px; display: flex; align-items: center; gap: 12px; margin-top: 14px;">
        ...
        <div class="footer-col">
          ...
          <ul class="footer-links">
            <li><a href="{prefix}pasar.html">Direktori Pasar Daerah</a></li>
...
def replace_footer_in_file(filepath, prefix=""):
    if not os.path.exists(filepath):
        return
    with open(filepath, "r", encoding="utf-8") as f:
        content = f.read()

    new_footer = get_exact_footer(prefix)

    # Cari awal <footer dan akhir </footer>
    f_start = content.find("<footer")
    f_end = content.rfind("</footer>")

    if f_start != -1 and f_end != -1:
        before = content[:f_start]
        after = content[f_end + len("</footer>"):]
        content = before + new_footer + after
    else:
        # Jika belum ada footer, masukkan sebelum script
        s_idx = content.find("<script")
        if s_idx != -1:
            content = content[:s_idx] + new_footer + "\n\n  " + content[s_idx:]
        else:
            b_idx = content.find("</body>")
            if b_idx != -1:
                content = content[:b_idx] + new_footer + "\n" + content[b_idx:]
...
def run_sync():
    ...
    for rf in root_files:
        replace_footer_in_file(rf, "")

    if os.path.exists("berita"):
        for d in os.listdir("berita"):
            sub_index = os.path.join("berita", d, "index.html")
            if os.path.isfile(sub_index):
                replace_footer_in_file(sub_index, "../../")

    if os.path.exists("pasar"):
        for d in os.listdir("pasar"):
            sub_index = os.path.join("pasar", d, "index.html")
            if os.path.isfile(sub_index):
                replace_footer_in_file(sub_index, "../../")
