...
MASTER_FOOTER = '''  <!-- FOOTER RESMI KEDINASAN -->
...
def apply_footer(filepath, prefix=""):
    ...
    # Cek apakah ada footer eksisting
    if "<footer" in html:
        footer_pattern = r'<footer.*?</footer>'
        html = re.sub(footer_pattern, rendered_footer, html, flags=re.DOTALL)
    else:
        # Sisipkan sebelum script pertama atau sebelum </body>
        if "<script" in html:
            html = re.sub(r'(<script)', rendered_footer + "\n\n  \\1", html, count=1)
        elif "</body>" in html:
            html = html.replace("</body>", rendered_footer + "\n</body>")

    with open(filepath, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"Applied master footer to: {filepath}")
