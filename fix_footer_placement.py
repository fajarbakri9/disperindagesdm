...
MASTER_FOOTER = '''  <!-- ==========================================================================
       FOOTER RESMI KEDINASAN (100% IDENTIK & KONSISTEN SE-WEBSITE)
       ========================================================================== -->
...
def fix_file(filepath, prefix=""):
    ...
    rendered = MASTER_FOOTER.replace("{prefix}", prefix)

    # Hapus SEMUA tag footer yang ada
    html = re.sub(r'<footer.*?</footer>', '', html, flags=re.DOTALL | re.IGNORECASE)

    # Letakkan tepat sebelum skrip-skrip terbawah atau sebelum </body>
    # Cari indeks blok skrip terbawah atau </body>
    body_close = html.rfind("</body>")
    if body_close != -1:
        # Cari skrip terakhir sebelum </body>
        # Cari tag <script yang paling dekat sebelum </body>
        last_scripts_match = re.search(r'(\s*<!-- CORE SCRIPTS.*|<!-- Scripts.*|<script.*)', html[body_close-1000:body_close], re.DOTALL | re.IGNORECASE)
        if last_scripts_match:
            insert_pos = (body_close - 1000) + last_scripts_match.start()
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"Correctly placed footer in: {filepath}")
